...
//   // Fetch hub details when hubId changes
//   useEffect(() => {
//     if (hubId) {
...
//       if (response.ok) {
//         // Extract hubs data - handle different response formats
//         const hubsData = data.data || data.hubs || data || [];
...
//   const fetchHubDetails = async (id) => {
//     setHubDetailsLoading(true);
//     try {
//       const response = await fetchWithTokenRefresh(`${API_CONFIG.HUB_DETAILS_API}/${id}`, {
//         method: 'GET'
//       });

//       const data = await response.json();
//       console.log('Hub details:', data);

//       if (response.ok) {
//         setSelectedHub(data.data || data);
//       } else {
//         console.error('Failed to fetch hub details:', data);
//         setSelectedHub(null);
//       }
//     } catch (error) {
...
//     // Reset pagination and fetch filtered results
//     setPagination({
//       before: null,
//       before_id: null,
//       limit: 10,
...
//       total: 0
//     });
//     // In real implementation, add search param to API
...
//   // Hub Details View
//   const HubDetailsView = () => {
//     if (hubDetailsLoading) {
//       return (
//         <div className="flex items-center justify-center h-64">
//           <div className="flex flex-col items-center gap-3">
//             <Loader2 className="w-8 h-8 text-green-600 animate-spin" />
//             <p className="text-gray-500">Loading hub details...</p>
//           </div>
//         </div>
//       );
//     }
...
//                 </div>
//                 <div>
//                   <h2 className="text-2xl font-bold text-gray-900">{selectedHub.name}</h2>
//                   <div className="flex items-center gap-3 mt-1 flex-wrap">
//                     <span className={`px-3 py-1 rounded-full text-xs font-medium border flex items-center gap-1 ${getStatusColor(selectedHub.status)}`}>
//                       {getStatusIcon(selectedHub.status)}
//                       {selectedHub.status || 'PENDING'}
//                     </span>
//                     <span className="text-sm text-gray-500">
//                       ID: {selectedHub.id || selectedHub.hub_id || 'N/A'}
//                     </span>
//                   </div>
//                 </div>
//               </div>
...
//                   <p className="text-sm font-medium text-gray-900">{selectedHub.location || selectedHub.address || 'N/A'}</p>
//                 </div>
//               </div>
...
//                   <p className="text-sm font-medium text-gray-900">{selectedHub.email || 'N/A'}</p>
//                 </div>
//               </div>
//             </div>
...
//                   <p className="text-sm font-medium text-gray-900">{selectedHub.phone || 'N/A'}</p>
//                 </div>
//               </div>
...
//                   <p className="text-sm font-medium text-gray-900">{formatDate(selectedHub.created_at)}</p>
//                 </div>
//               </div>
...
//                   <p className="text-sm font-medium text-gray-900">{formatDate(selectedHub.updated_at)}</p>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//                   <p className="text-sm font-medium text-gray-900">{selectedHub.address}</p>
//                 </div>
//                 <div>
//                   <p className="text-xs text-gray-500">Pincode</p>
//                   <p className="text-sm font-medium text-gray-900">{selectedHub.pincode || 'N/A'}</p>
//                 </div>
//                 <div>
//                   <p className="text-xs text-gray-500">Total Stations</p>
//                   <p className="text-sm font-medium text-gray-900">{selectedHub.total_stations || selectedHub.station_count || 0}</p>
//                 </div>
//               </div>
//             </div>
//           )}
...
//                 onClick={() => navigate(`/manage-hubs/${hub.id || hub.hub_id}`)}
//               >
...
//                         <p className="text-sm text-gray-500">{hub.city || hub.location || 'N/A'}</p>
//                       </div>
//                     </div>
//                     <span className={`px-2 py-1 rounded-full text-xs font-medium border flex items-center gap-1 ${getStatusColor(hub.status)}`}>
...
//                     <span className="text-gray-600 truncate">{hub.location || hub.address || 'N/A'}</span>
//                   </div>
                  
//                   <div className="flex items-center gap-2 text-sm">
//                     <Globe className="w-4 h-4 text-gray-400 flex-shrink-0" />
//                     <span className="text-gray-600">
//                       {hub.city && hub.state ? `${hub.city}, ${hub.state}` : 'N/A'}
//                     </span>
//                   </div>
...
//                         <p className="text-sm font-semibold text-gray-900">{hub.total_stations || hub.station_count || 0}</p>
//                       </div>
//                     </div>
...
//         {/* Content */}
//         <div className="p-6">
//           {hubId ? <HubDetailsView /> : <HubsListView />}
//         </div>
//       </div>
//     </div>
//   );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredHubs.map((hub) => (
              <div 
                key={hub.id || hub.hub_id} 
                className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow overflow-hidden cursor-pointer"
                onClick={() => navigate(`/hub-details/${hub.id || hub.hub_id}`)}
              >
                <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-green-50 to-emerald-50">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
