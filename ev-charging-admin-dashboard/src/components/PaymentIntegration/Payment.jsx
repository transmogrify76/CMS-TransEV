...
const CPOPaymentIntegration = () => {
  ...
  const fetchIntegration = useCallback(async () => {
    ...
    try {
      const response = await fetchWithTokenRefresh(`${API_CONFIG.INTEGRATIONS_API}/RAZORPAY`, {
        method: 'GET'
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`${API_CONFIG.INTEGRATIONS_API}/RAZORPAY`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'X-CPO-App-ID': CPO_APP_ID
  ...
  <div className="bg-gradient-to-r from-green-50 to-emerald-50 px-6 py-4 border-b border-green-200 flex items-center justify-between">
    ...
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        className="px-4 py-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition flex items-center gap-2 text-sm font-medium shadow-lg shadow-green-500/25 hover:shadow-green-500/40"
      >
        <Edit size={16} />
        Update
      </button>
